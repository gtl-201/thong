...
const ItemList: React.FC<ItemListProps> = ({ img, rating, price }) => {
    ...
    return (
        <div className="flex flex-wrap mt-[6rem] justify-center">
            <div onClick={() => handleClick()} className='flex-wrap shadow-sm bg-[#FDE9DE] m-10 w-[13rem] h-[15.5rem] rounded-[2rem] relative'>
                ...
                <div className='absolute bottom-0 mx-4 my-5'>
                    <div className='font-Fredoka font-bold text-[22px] text-wrap tracking-wide capitalize'>Title here</div>
                    <div className='flex'>
                        {rateStar.map((item) => {
                            console.log(item);
                            return (
